...
interface PainPointsHierarchyTableProps {
...
export function PainPointsHierarchyTable({
  selectedStatuses = null,
  focusedIssueKey = null,
  noPainPointSubStepsOnly = false,
}: PainPointsHierarchyTableProps) {
  ...
  const painPointsBySubStep = useMemo(
    () => buildPainPointsBySubStep(cards, painPointRecords, selectedStatuses),
    [cards, painPointRecords, selectedStatuses],
  );
  ...
  const visibleSubStepIds = useMemo(() => {
    ...
    if (noPainPointSubStepsOnly) {
      return new Set(findSubStepIdsWithoutPainPoints(cards, subSteps));
    }
    if (selectedStatuses !== null) {
      return new Set(findSubStepIdsForStatusFilter(cards, painPointRecords, selectedStatuses));
    }
    return null;
  }, [cards, focusedIssueKey, noPainPointSubStepsOnly, painPointRecords, selectedStatuses, subSteps]);

  const collapseMessage = useMemo(() => {
    if (focusedIssueKey) {
      return `Showing sub-steps for ${focusedIssueKey}. Click the pain point again to show all.`;
    }
    if (noPainPointSubStepsOnly) {
      return 'Showing sub-steps with no pain point on the board.';
    }
    if (selectedStatuses !== null) {
    ...
    return null;
  }, [focusedIssueKey, noPainPointSubStepsOnly, selectedStatuses]);
        ...
        <p className="px-4 py-6 text-center text-sm text-neutral-600">
          {focusedIssueKey
            ...
            : noPainPointSubStepsOnly
              ? 'No sub-steps without pain points.'
      ...
      <div className="grid grid-cols-2 gap-x-4">
        {rows.map((row, index) => {
          const subStepIssueKeys =
            row.level === 'subStep' ? (painPointsBySubStep.get(row.id) ?? []) : [];
          const issueKeys =
            ...
            <Fragment key={row.id}>
              ...
              >
                {issueKeys.map((issueKey) => {
                  const status = painPointRecords[issueKey]?.status?.trim() ?? null;
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          painPointStatusDotClass(status),
                        )}
                        aria-hidden="true"
                      />
